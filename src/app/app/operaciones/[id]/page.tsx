...
type CardItem = { label: string; value: string; detail?: string; highlight?: boolean };
type AssumptionItem = { id: string; label: string; value: string; source?: string };
...
export default async function OperacionDetallePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const qj = (quote.quoteJson ?? {}) as {
    cards?: CardItem[];
    assumptions?: AssumptionItem[];
  };

  const title = pj.title ?? pj.displayTitle ?? quote.userText ?? "Operación";
  const ncm = pj.ncm ?? null;
  const origin = pj.origin ?? null;
  const cards: CardItem[] = Array.isArray(qj.cards) ? qj.cards : [];
  const assumptions: AssumptionItem[] = Array.isArray(qj.assumptions) ? qj.assumptions : [];

  const total = fmtUsdRange(quote.totalMinUsd, quote.totalMaxUsd);
  ...
  return (
    <div className="relative px-safe pb-8 pt-4 sm:p-6 lg:p-8">
      ...
      <div className="relative mx-auto max-w-[1000px]">
        {/* Breadcrumb */}
        <nav aria-label="Navegación" className="flex items-center gap-1.5 text-[12px]">
          <Link href="/app/operaciones" className="text-[#555c6b] transition-colors hover:text-[#aab4c2]">Operaciones</Link>
          <span className="text-[#333d4d]" aria-hidden>/</span>
        ...
        <div className="relative mt-4 overflow-hidden rounded-2xl border border-white/[0.06] bg-[#0B1622]/80 px-5 py-5 backdrop-blur sm:px-6">
          ...
          <div className="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0 flex-1">
              ...
              <h1
                className="mt-1.5 text-[clamp(1.1rem,4vw,1.35rem)] font-extrabold leading-tight tracking-tight text-white"
              ...
              <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
                ...
                <span className="rounded-full border border-white/[0.06] bg-white/[0.04] px-2.5 py-0.5 text-[10px] font-medium text-[#8a96a8]">
                  {quoteStageLabel(quote.stage)}
                </span>
                {ncm && (
                  <span className="rounded-full border border-[#18C3D6]/20 bg-[#18C3D6]/[0.08] px-2.5 py-0.5 text-[10px] font-bold text-[#18C3D6]">
                    NCM {ncm}
                  </span>
                )}
                {origin && (
                  <span className="rounded-full border border-white/[0.06] bg-white/[0.04] px-2.5 py-0.5 text-[10px] text-[#555c6b]">
                    Origen: {origin}
                  </span>
                )}
              </div>
              <p className="mt-2 text-[11px] text-[#4a5568]">
                Creada {fmtActivity(quote.createdAt)} · ID{" "}
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_280px]">
          ...
          <div className="space-y-6">
            {/* Cost cards */}
            {cards.length > 0 && (
              <section aria-label="Desglose de costos">
                <div className="flex items-center gap-2">
                  <div className="h-px w-3 bg-[#18C3D6]/40" aria-hidden />
                  ...
                  {cards.map((c) => (
                    <div
                      ...
                      className={`relative overflow-hidden rounded-xl border p-4 ${
                        c.highlight ? "border-[#d4a843]/20 bg-[#d4a843]/[0.04]" : "border-white/[0.05] bg-[#0B1622]"
                      }`}
                    >
                      <div
                        className={`absolute left-0 top-3 bottom-3 w-[3px] rounded-r-full ${
                          c.highlight ? "bg-[#d4a843]/50" : "bg-[#18C3D6]/15"
                        }`}
                        aria-hidden
                      />
                      <p className="pl-3 text-[10px] font-medium uppercase tracking-wider text-[#555c6b]">{c.label}</p>
                      <p
                        className={`mt-1 pl-3 text-[15px] font-bold ${c.highlight ? "text-[#d4a843]" : "text-white"}`}
                        style={{ fontFamily: "var(--font-display)" }}
                      >
                        {c.value}
                      </p>
            ...
            {assumptions.length > 0 && (
              <section aria-label="Supuestos de la cotización">
                <div className="flex items-center gap-2">
                  <div className="h-px w-3 bg-[#18C3D6]/40" aria-hidden />
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {assumptions.map((a) => (
                    <span
                      key={a.id}
                      className="rounded-full border border-white/[0.06] bg-[#0B1622] px-3 py-1 text-[11px] text-[#8a96a8]"
                    >
                      {a.label}:{" "}
                      <span className="font-semibold text-white">{a.value}</span>
                      {a.source && <span className="ml-1 text-[#4a5568]">({a.source})</span>}
                    </span>
                  ))}
                </div>
              </section>
            ...
            {/* User text */}
            <section aria-label="Descripción original">
              <div className="flex items-center gap-2">
                <div className="h-px w-3 bg-[#18C3D6]/40" aria-hidden />
                <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#7a8599]">Descripción original</p>
              </div>
              <div className="mt-3 rounded-xl border border-white/[0.04] bg-[#0B1622] p-4">
                ...
                {quote.sourceUrl && (
                  <a
                    ...
                    className="mt-2 inline-block break-all text-[12px] text-[#18C3D6] hover:underline"
                  >
                    {quote.sourceUrl.length > 60 ? `${quote.sourceUrl.slice(0, 60)}…` : quote.sourceUrl}
                  </a>
                )}
              </div>
            </section>
          ...
          <aside className="space-y-3">
            <div className="relative overflow-hidden rounded-xl border border-[#d4a843]/15 bg-[#d4a843]/[0.04] p-5">
              <div className="absolute left-0 top-3 bottom-3 w-[3px] rounded-r-full bg-[#d4a843]/50" aria-hidden />
              <p className="pl-3 text-[10px] font-semibold uppercase tracking-wider text-[#7a6a3a]">Total estimado</p>
              <p
                className="mt-1 pl-3 text-[22px] font-extrabold text-[#d4a843]"
                style={{ fontFamily: "var(--font-display)" }}
              >
                {total}
              </p>
            ...
            <div className="rounded-xl border border-white/[0.04] bg-[#0B1622] p-5">
              ...
              <div className="mt-3 space-y-2">
                {canIniciarImportacion ? <IniciarImportacionButton quoteId={quote.id} /> : null}
                {quote.operation ? (
                  <Link
                ...
                <a
                  href={pdfHref}
                  aria-label="Descargar cotización en PDF"
                  className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-[#18C3D6] py-2.5 text-[12px] font-semibold text-[#030d18] transition-colors hover:bg-[#0ea5b9]"
                >
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                    <path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                  </svg>
                ...
                <Link
                  ...
                  className="flex w-full items-center justify-center rounded-lg border border-white/[0.06] py-2 text-[12px] text-[#6b7a8d] transition-colors hover:border-white/[0.12] hover:text-[#aab4c2]"
                >
                  ← Volver
                </Link>
              </div>
