...
export default function OperacionDetalleError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("[OperacionDetalleError]", error);
  }, [error]);
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center px-6 text-center">
      ...
      <p className="mt-2 max-w-xs text-[13px] leading-relaxed text-[#5a6577]">
        Ocurrió un error al cargar el detalle. Intentá de nuevo en unos segundos.
      </p>
      {error.digest ? (
